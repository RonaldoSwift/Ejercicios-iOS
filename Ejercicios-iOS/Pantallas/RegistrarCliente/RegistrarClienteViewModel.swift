...
class RegistrarClienteViewModel{
    ...
    func obtenrTodosLosClientes() async {
        do{
            let clientes = try await getAll()
            print( clientes)
        }catch{
    ...
    func crearCliente(dni:Int, nombreCliente:String, direccion:String, distrito:String) async {
        do{
            try await crear(dni: dni, nombreCliente: nombreCliente, direccion: direccion, distrito: distrito)
        } catch{
